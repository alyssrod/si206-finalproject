...
cur.execute("CREATE TABLE IF NOT EXISTS venue_ids(id INTEGER primary key, venue_name UNIQUE)")
conn.commit()
...
def add_venue_id_table(venue,cur,conn):
    venue_name=venue
    cur.execute("INSERT OR IGNORE INTO venue_ids (venue_name) VALUES (?)", (venue_name,))
    conn.commit()

limiter=0
for item in result_setlists:
    if limiter>10:
        break
    else:
        venue=item["venue"]["name"]
        add_venue_id_table(venue,cur,conn)
        limiter+=1
